...
int ledPin = 13; // to indicate it is reading data
#include "Tlc5940.h";
 int channel = 0; // kanal tlc 0-47
 int value = 0; // wartosc na kanal e 0-4095
 int rawdata[16];
//buffer size for NMEA compliant GPS string
//For Razor, set value to 17 instead
#define DATABUFFERSIZE 10 //nie wiem jak dlugi jest moje zdanie??
char dataBuffer[DATABUFFERSIZE+1]; //Add 1 for NULL terminator
char startChar = '!'; // or whatever your start character is zacznij spacja
char endChar = '\n';           //'\r' skonczy endline
boolean storeString = false; //This will be our flag to
        
       void setup()
{
   Serial.begin(4800); //Baud set at 9600 for compatibility, CHANGE!
  Tlc.init(); //dajcie troche swiatla na poczatek
...
if(getSerialString()){
  for (int i = 0; i < DATABUFFERSIZE; i++) {
    ...
    Serial.println(dataBuffer[i]);      
      digitalWrite(ledPin, HIGH); // this will be in char
}
} else {
for (int j = 0; j < 11; j++) {
rawdata[j] = Serial.read();                  // this should be int
    Serial.println(rawdata[j]);      
       }
      //   Serial.println(rawdata);
      
//  channel = dataBuffer[5]; // kanal tlc 0-47
// value = dataBuffer[12]  + dataBuffer[14]; 
    ...
    while(Serial.available()>0){
        ...
        if(storeString){
            ...
            if(incomingbyte==endChar){
                ...
                //Our data string is complete.  retur  
                return true;
            }
            else{
...
rawdata =  Serial.read(); // czytaj
Serial.println(rawdata); i pokaz co przychodzi
Serial.println('\n');
     if (Serial.read() == ('\n')) {
    // channel = Serial.read();
...
A note on strings
The Arduino provides two methods of storing character data.  You have the standard C style character arrays, and you have Arduino's own attempt at a C++ String class.  C style character arrays require a bit more effort on the programmers part while Arduino's String class attempts to provide an easier to use object that handles most of the string manipulation under the hood.  There is one potentially significant pitfall with the String class that you need to be aware of when using it.  The class relies on dynamically 
allocating buffers to handle different string sizes as well as changes in string sizes.  With the Arduino's limited SRAM, repeated dynamic memory allocation will fragment memory and eventually cause the Arduino to behave unpredictably or lock up completely.  It is for this reason that I utilize C style character arrays.  I'm not saying the String class can't be used, but steps need to be taken to keep it from fragmenting memory at which point you lose much of the advantage the class tries to provide.

...
void random_animation(){//  random animation  random animation  random animation
   ...
   int speed_swipe = random(1, 10);
   if(pick_color>3)
   pick_color=1;
   
   
   if(pick_color==1){
    // while(abs(oldred-red)<50)
   red = random(0, pick_max); 
   } 
   if(pick_color==2){
    // while(abs(oldblue-blue)<50)
   blue = random(0, pick_max);
   } 
   if(pick_color==3){
    // while(abs(oldgreen-green)<50)
...
spibit=0;
  ...
  PORTD &= 0<<2;//XLAT data is in now
  PORTD &= 0<<4;//VPRG is good to go into normal mode LOW
